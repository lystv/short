...
(async () => {
	...
	res2.forEach((item) => {
      ...
      item.newName = '(OK版)5.1+TV内测java-32.apk';
		...
		res3 = (
			await axios.get(
				'https://444.lytv.gq/repos/fongmi/release/contents/apk/kitkat?ref=main')
		).data.filter(
			(item) =>
				item.name.endsWith('.apk')
         );
...
res4.forEach((item) => {
  ...
  item.newName = '(FM版)5.1+TV正式java-32.apk';
   ...
   res5 = (
    ...
    // 添加部分：遍历res5，修改部分文件名并给出下载链接
res5.forEach((item) => {
    if (item.name === 'mobile-python-arm64_v8a.apk') {
  item.newName = '(FM版)手机内测py-64.apk';
    } else if (item.name === 'mobile-java-arm64_v8a.apk') {
  item.newName = '(FM版)手机内测java-64.apk';
    } else if (item.name === 'mobile-python-armeabi_v7a.apk') {
  item.newName = '(FM版)手机内测py-32.apk';
    } else if (item.name === 'mobile-java-armeabi_v7a.apk') {
  ...
  item.newName = '(FM版)5.1+TV内测java-32.apk';
	...
	let res = res1.concat(res2,res3,res4,res5);
	...
	res.forEach((item) => {
        ...
        <a class="col-span-2" href="https://ghproxy.net/${item.download_url}">
            <span>
